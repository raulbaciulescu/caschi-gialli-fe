...
const NotificationSettings: React.FC = () => {
  ...
  const isCustomer = user?.type === 'client' || user?.type === 'customer';

  useEffect(() => {
    // Only load preferences for CG users
    if (!isCustomer) {
      loadPreferences();
    }
  }, []);
  ...
  // For customers, show only delete account section
  if (isCustomer) {
    return (
      <div className="space-y-8">
        ...
        {/* Delete Account Section for Customers */}
  ...
  // For CG users, show full notification settings
  return (
    <div className="space-y-8">
