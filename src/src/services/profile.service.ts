...
class ProfileService {
    ...
    public async updateCGProfile(updates: ProfileUpdateRequest): Promise<User> {
        ...
        const response = await httpService.put<User>(
            API_ENDPOINTS.CG.UPDATE_PROFILE, // Use CG specific endpoint
            formData,
            {
                headers: {
    ...
    public async uploadProfileImage(file: File): Promise<string> {
        ...
        const response = await httpService.post<{ profileImageUrl: string }>(
            `${API_ENDPOINTS.CG.PROFILE}/upload-image`, // Use CG specific endpoint
            formData,
            {
                headers: {
                    'Content-Type': 'multipart/form-data',
    ...
    public async uploadGalleryImages(files: File[]): Promise<string[]> {
        ...
        const response = await httpService.post<{ galleryImageUrls: string[] }>(
            `${API_ENDPOINTS.CG.PROFILE}/upload-gallery`, // Use CG specific endpoint
            formData,
            {
                headers: {
                    'Content-Type': 'multipart/form-data',
     ...
     * Get CG profile data
     */
    public async getCGProfile(): Promise<User> {
        const response = await httpService.get<User>(API_ENDPOINTS.CG.PROFILE); // Use CG specific endpoint
        return response;
    }

    /**
     * Get general user profile data (for clients)
     */
